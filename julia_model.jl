...
# ╔═╡ bd08828e-1e31-11f1-a576-4582a3035150
using Turing, Distributions, DataFrames, CSV, StatsBase, Random

# ╔═╡ bb16c8d3-4eae-4cab-a876-628989ea07bc
using MCMCChains: Chains

# ╔═╡ 3689a967-2cc6-48e0-bdb4-e7d61c0f645e
df = CSV.read("./all_scores.csv", DataFrame)

# ╔═╡ d1c35f93-0c39-417b-8d99-ae013149ce7b
@model function hierarchical_archery(y, archery_idx, event_idx, N_arch, N_event)
	# Hyperparameters
	μ      ~ Normal(500, 80)
	τ_arch ~ truncated(Normal(0, 80), 0, Inf)
	τ_event ~ truncated(Normal(0, 40), 0, Inf)
	σ       ~ truncated(Normal(0, 50), 0, Inf)

	# Varying effects (centered paramaterisation)
	z_arch  ~ filldist(Normal(), N_arch)
	z_event ~ filldist(Normal(), N_event)

	θ_arch    = μ .+ τ_arch .* z_arch
	α_event   = τ_event .* z_event

	for i in eachindex(y)
		y[i] ~ truncated(Normal(θ_arch[archery_idx[i]] + α_event[event_idx[i]], σ), 0, 600)
	end
end
	

# ╔═╡ 71716988-391a-4fff-9ad9-e1a97f24207c
archer_levels = groupby(df, :archer) |> keys |> collect |> x->sortperm(x)

# ╔═╡ b75e40d3-9104-4f3c-ac8e-8bb4ef71afee
event_levels = groupby(df, :event_date) |> keys |> collect |> x-> sortperm(x)

# ╔═╡ 3916f461-5238-4a03-a6a1-2ef91b3a5734
chain = sample(hierarchical_archery(df.score, archer_levels, event_levels, length(archer_levels), length(event_levels)), NUTS(0.65), 2000)
...
manifest_format = "2.0"
project_hash = "3df3231adfb449ebdaa4e5e020ea9de33af57d8b"
...
git-tree-sha1 = "b0bc153311e9d57db3c1cb72d6584e553bdff719"
...
git-tree-sha1 = "9ece6aebbdf69503559aed230b3b10c9b07714af"
